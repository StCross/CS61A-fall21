...
def pingpong(n):
    ...
    def iter(i, step, index):
        if index == n:
            return i
        if num_eights(index) or index % 8 == 0:
            return iter(i - step, -step, index + 1)
        else:
            return iter(i + step, step, index + 1)
...
def count_coins(change):
    ...
    True
    """
    def count_util(change, max_coin_value):
        if change in [0, 1]:
            return 1
        elif change < 0:
            return 0
            ...
            return count_util(change - max_coin_value,
                              max_coin_value) + count_util(
                                  change, descending_coin(max_coin_value))

    return count_util(change, 25)
