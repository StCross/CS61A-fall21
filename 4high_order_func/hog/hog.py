...
def picky_piggy(score):
    if score == 0:
        return 7
    con = 142857
    score = score % 6 if score % 6 > 0 else 6
    num = 6 - score + 1
    iter = 0
    while iter < num:
        remainder = con % 10
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(now_score0, now_score1, running_high=running_high):
        if who == 0:
            now_score = now_score0
        else:
            now_score = now_score1
        now_compare = now_score - last_score
        if now_compare > running_high:
            running_high = now_compare
            print(
                f"{running_high} point(s)! That's a record gain for Player {who}!"
            )
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_score = 0
    for i in range(1, 11):
        now_score = make_averaged(roll_dice, trials_count)(i, dice)
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    score_gain = picky_piggy(opponent_score)
    if score_gain >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_gain = picky_piggy(opponent_score)
    if score + score_gain == opponent_score:
        score_gain += opponent_score
    if score_gain >= cutoff:
        return 0
    else:
        return num_rolls
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    first start with hog pile strategy
    then, if roll normal dices gain more than average hog pile strategy caculated by make_avereged(), roll normal dices, else choose hog pile strategy
    when lead, take fewer risk by choosing num_rolls from 1 to 2, else choosing from 1 to 5
    """
    # BEGIN PROBLEM 12
    if score == 0 and opponent_score == 0:
        return 0
    elif score > opponent_score:
        hog_pile_gain = hog_pile(score, opponent_score)
        normal_roll_gain = 0
        normal_num = 0
        for i in range(1, 3):
            now_roll_gain = make_averaged(roll_dice, 10)(i, six_sided)
            if now_roll_gain > normal_roll_gain:
                normal_roll_gain = now_roll_gain
                normal_num = i
        if normal_roll_gain > hog_pile_gain:
            return normal_num
        elif normal_roll_gain <= hog_pile_gain:
            return 0
    else:
        hog_pile_gain = hog_pile(score, opponent_score)
        normal_roll_gain = 0
